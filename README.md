...
## Shortcuts
- **⌥Space** — Click mode: every clickable element gets a label; type it to click. Hold a modifier while finishing the label for a variant — **⌘** command-click, **⌃** right-click — or type the label twice for a double-click. Labels are stable (the same element keeps the same label across activations — they're derived from its position), spatial (the first letter grows alphabetically from the screen's top-left toward bottom-right, so a label hints at where its element is), and ergonomic (scored by a per-key reach model: hard-to-reach keys like q/p/b/x/z are never used, same-finger reaches are excluded, and hand alternation, home-row and resting keys are preferred).
- **⌥⇧Space** — Scroll mode: every scroll area is outlined and numbered; the active one is highlighted. **i/k** up/down and **j/l** left/right (inverted-T), **⇧** dashes, **d/u** or **⌃D/⌃U** smooth half-page, **g/G** top/bottom, **Tab/⇧Tab/⌃N/⌃P/1–9** switch areas. Esc exits.
